...
def transform(obj, translation=None, rotation=None, scale=None, local=False):
    tf = np.eye(4)
    if scale is not None:
        tf[:3, :3] = np.diag(scale)
    if rotation is not None:
        tf[:3, :3] = np.array(mathutils.Euler(rotation).to_matrix()) @ tf[:3, :3]
    if translation is not None:
        tf[:3, 3] = translation
    tf = mathutils.Matrix(tf)
    if local:
        obj.matrix_world = obj.matrix_world @ tf
    else:
        obj.matrix_world = tf @ obj.matrix_world
